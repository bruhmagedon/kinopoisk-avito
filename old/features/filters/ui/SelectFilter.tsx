...
import type { FilterApiResponse } from "old/entities/filters";
import { setFilter } from "old/entities/filters";
            ...
            <Listbox.Options className='absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-md bg-input-bg py-1 text-base text-white shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm'>
              {filterData.map((item, index) => (
                <Listbox.Option
                  key={index}
                  className={({ active }) =>
                    `relative cursor-default select-none py-2 pl-10 pr-4 ${active ? "bg-primary text-black" : "text-white"
                    }`
                  }
                  value={item.name}
                >
